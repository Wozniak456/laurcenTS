        ...
        <div className='flex flex-wrap items-center justify-between gap-4'>
            <div className='w-full sm:w-1/3'>
                <Select 
                    ...
                    defaultSelectedKeys={[
                        batches.filter(batch => batch.name === poolInfo.batch?.name).map(batch => String(batch.id))[0]
                    ]}
                    isDisabled={!editionAllowed}
                >
                    {batches.map(batch => (
                        <SelectItem key={Number(batch.id)} value={String(batch.id)}>{batch.name}</SelectItem>
                    ))}
                </Select>
            </div>
            ...
            {formState && formState.message && (
                <div className={`w-full my-2 p-2 border rounded ${formState.message.includes('успішно') ? 'bg-green-200 border-green-400' : 'bg-red-200 border-red-400'}`}>
                    {formState.message}
                </div>
            )}
        </div>
