...
export default function PurchTableComponent({
  purchtables,
  vendors,
  items,
}: PurchTableComponentProps) {
  ...
  const [showCreatePurchHeaderModal, setShowCreatePurchHeaderModal] =
  ...
  const [linesList, setLinesList] = useState<line[] | undefined>(
        ...
        <TableBody>
          {purchtables
            ...
            .map((header) => (
              <TableRow
                key={header.id}
                onClick={() => handleRowSelection(header.id)}
              >
