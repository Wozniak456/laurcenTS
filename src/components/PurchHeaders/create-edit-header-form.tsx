...
export default function CreateEditPurchHeaderForm({
  ...
  const [formState, action] = useFormState(
    header ? actions.editPurchtable : actions.createPurchTable,
    { errors: {} }
  );
  ...
  return (
    <div className="w-full">
      ...
      {header ? (
        <h2 className="text-lg font-semibold m-4">Редагування накладної</h2>
      ) : (
        <h2 className="text-lg font-semibold m-4">Нова накладна</h2>
      )}
      ...
      <form className="m-4" action={action} onSubmit={handleCloseModal}>
        <div className="flex flex-row flex-wrap gap-4 justify-between items-start my-8">
          <Input
            ...
            defaultValue={
              header ? header.date_time.toISOString().split("T")[0] : ""
            }
          />
          ...
          <Select
            ...
            defaultSelectedKeys={
              header && header.vendors && header.vendors.name
                ? [
                    vendors
                      .filter((vendor) => vendor.name === header.vendors.name)
                      .map((vendor) => String(vendor.id))[0],
                  ]
                : undefined
            }
          >
