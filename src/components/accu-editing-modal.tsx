...
export default function EditAccumulation({
  columns,
  selectedRow,
}: EditAccumulationProps) {
  const [initialValues] = useState<RowData>(selectedRow);

  const [formData, setFormData] = useState<RowData>(selectedRow);

  // Update form state
  const [formState, action] = useFormState(actions.updateAccumulation, {
    message: "",
  });
          ...
          if (col.key !== "location_name" && col.key !== "total") {
            return (
              <div key={col.key} className="mb-4">
                ...
                <input
                  onChange={(e) => {
                    updateItem(col.key, e.target.value);
                  }}
                  className="w-full px-2 py-1 border rounded"
                ...
                {initialValues[col.key] !== formData[col.key] && (
                  <>
                    ...
                    <input
                      type="hidden"
                      name={`prev_item_${col.key}`}
                      value={initialValues[col.key]}
                    />
                  </>
                )}
              </div>
