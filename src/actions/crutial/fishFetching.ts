...
export async function fishFetching(
    ...
    formData: FormData
        ...
        const fishingData = [
            { amount: commercial_fishing_amount, total_weight: commercial_fishing_total_weight, reason: FetchingReasons.CommercialFishing },
            { amount: sorted_fishing_amount, total_weight: sorted_fishing_total_weight, reason: FetchingReasons.Sorted },
            { amount: growout_fishing_amount, total_weight: growout_fishing_total_weight, reason: FetchingReasons.GrowOut },
            { amount: more500_fishing_amount, total_weight: more500_fishing_total_weight, reason: FetchingReasons.MoreThan500 },
            { amount: less500_fishing_amount, total_weight: less500_fishing_total_weight, reason: FetchingReasons.LessThan500 }
        ];
        ...
        const result = await db.$transaction(async (prisma) => {
            try {
                ...
                for (const { amount, total_weight, reason } of fishingData) {
                    if (amount) {
                        const result = await createFishingTransaction(
                            prisma, Number(fetchDoc.id), location_id_from, 88, batch_id_from,
                            amount, 1, reason, total_weight, week_num
                        );
                ...
                if (first_parent_generation) {
                    ...
                    await Promise.all(grouped_first_ancestor.map(async record => {
                ...
                throw new Error('Щось пішло не так під час виконання транзакції', innerError); // Кидаємо помилку для відкату
            }
        },{timeout: 20000});
