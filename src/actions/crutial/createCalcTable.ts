    ...
    prisma?: any){
        ...
        try{
            ...
            if (average_fish_mass < 25){
                await createCalcBelow25(fish_amount, average_fish_mass, percentage, doc_id, prisma)
                await setTransitionDayForLocation(location_id_to, prisma)
            }
            else{
                await createCalcOver25(fish_amount, average_fish_mass, percentage, doc_id, prisma)
                await setTransitionDayForLocation(location_id_to, prisma)
            }
