...
export default async function PurchTableShowPage(
  props: PurchTableShowPageProps
) {
  let purch_record;
  try {
    ...
    const deletePurchRecordAction = actions.deletePurchTable.bind(
      null,
      purch_record.id
    );
      ...
      <div>
        <div className="flex m-4 justify-between center">
          ...
          <div className="flex gap-4">
            <Link
              href={`/purchtable/${purch_record.id}/edit`}
        ...
        <div className="p-3 border rounded border-gray-200">
          ...
          <h2>{purch_record.date_time.toLocaleString()}</h2>
        </div>
        <div className="p-3 border rounded bg-gray-200 border-gray-200">
          <label>
            <b>Vendor_id:</b>
          </label>
          <h2>{purch_record.vendor_id}</h2>
        </div>
        <div className="p-3 border rounded border-gray-200">
          <label>
            <b>Vendor_doc_number:</b>
          </label>
          <h2>{purch_record.vendor_doc_number}</h2>
        </div>
      </div>
    );
  } catch (error) {
