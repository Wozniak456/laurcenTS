...
export default async function LeftoversPerPeriod(
  props: LeftoversPerPeriodProps
) {
  try {
    ...
    for (const area of areas) {
      for (const line of area.productionlines) {
        for (const pool of line.pools) {
          let aggregatedInfo: poolManagingType | undefined = await poolInfo(
            pool.locations[0].id,
            date
          );
        ...
        <table className="mb-4">
          <thead>
            <tr>
              <th
                colSpan={7}
                className="px-4 py-2 border border-gray-400 text-center font-normal whitespace-nowrap text-sm"
              >
                {date}
              </th>
            </tr>
            ...
            {areas.map((area, areaIndex) => (
              <React.Fragment key={areaIndex}>
                <tr>
                  <td
                    colSpan={7}
                    className="px-4 py-2 border text-center bg-blue-200 border-gray-400"
                  >
                    {" "}
                    {area.name}
                  </td>
                ...
                {area.productionlines.map((line, lineIndex) => (
                  <React.Fragment key={lineIndex}>
                    <tr>
                      <td
                        colSpan={7}
                        className="px-4 py-2 border text-center bg-blue-300 border-gray-400"
                      >
                        {" "}
                        {line.name}
                      </td>
