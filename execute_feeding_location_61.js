...
async function executeFeedingLocation61() {
  try {
    console.log("=== EXECUTING FEEDING FOR LOCATION 61 ===\n");

    const location_id = 61;
    const item_id = 8; // ALLER 6 mm bona (from our debugging)
    const date = "2025-08-13"; // Using an available date
    const executed_by = 3;
    ...
    // 3. Check for blocking posted operations
    console.log("\n3. Checking for blocking operations...");
    ...
    console.log("✅ No blocking operations found");
...
executeFeedingLocation61()
  .then(() => {
    console.log("\n🎉 Feeding execution completed successfully!");
    process.exit(0);
  })
  .catch((error) => {
